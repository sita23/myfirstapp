...
if (isset($_GET['id'])) {
    ...
    try {
        $request = $client->request("GET", "/api/product/" . $productId);
        $product = json_decode($request->getBody()->getContents(), true);
    ...
    if (isset($_POST['update'])) {
        try {
            $response = $client->put("/api/product/" . $productId, [
                'json' => [
                    'name' => $_POST['name'],
                    'stock' => $_POST['stock'],
        ...
        <div class="col-md-4">
            ...
            <div class="form-group">
                ...
                <input type="datetime-local" name="consumption_date" class="form-control" id="exampleInputPassword1"
                       placeholder="tüketim tarihi" value="<?=$product['consumption_date']?>">
            </div>
            <div class="form-group">
                ...
                <input type="datetime-local" name="production_date" class="form-control" id="exampleInputPassword1"
                       placeholder="üretim tarihi" value="<?=$product['production_date']?>">
            </div>
            <div class="form-group">
                <label for="exampleInputPassword1">Fiyat:</label>
                <input type="text" name="price" class="form-control" id="exampleInputPassword1"
                       placeholder="fiyat" value="<?=$product['price']." tl"?>">
            </div>
